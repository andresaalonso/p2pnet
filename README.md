...
Lo que he hecho es crear la clase P2PNet con todo estructurado para usarlo más fácil. Si queréis probarlo, podéis usar cualquiera de los dos archivos test. TEST_SINGLE.py es para probarlo con una foto especificada en el path, y TEST_STREAM.py le podéis poner la url de vuestra cámara. La parte de la homografía no se la he añadido al stream porque estaba probando como dejarla bien hecha, pero si se la queréis añadir funciona. En teoría debería descargarse todo automáticamente y funcionar en la primera ejecución.


```
git clone https://github.com/andresaalonso/p2pnet.git
cd p2pnet
pip install -r requirements.txt
```

Está es la versión que utilizo yo y me funciona (también está probada en python 3.12.9):

```
Mi version de python==3.8.10

torch==2.4.1
torchvision==0.19.1
tensorboardX==No la tengo y no falla
easydict==Tampoco la tengo y no falla
pandas==2.0.3
numpy==1.24.4
scipy==1.10.1
matplotlib==3.7.5
...
# Modificaciones para que funcionase (ya están aplicadas)

en el archivo

```
/CrowdCountingP2PNet/util/misc.py
```

Modificar la línea 25 por
...
En el archivo:
```
CrowdCountingP2PNet/models/vgg_.py
```
es necesario descargar vgg16_bn y vgg16 de los enlaces indicados, guardarlos donde quieras y modificar los paths en model_paths


En la linea 71 de 
```
CrowdCountingP2PNet/run_test.py
```
cambiar Image.ANTIALIAS por Image.LANCZOS



Ahora hay que descargar el backbone de los enlaces indicados en :
```
/models/vgg_.py
```
Es necesario descargar vgg16_bn y vgg16, y guardarlos en /weights/backbone/

Y debería estar todo funcionando. Yo ya tenía un entorno con todos los paquetes así que no me hizo falta instalar nada. Por lo tanto, no tengo ni idea de si os dará algún error instalarlo directamente desde los requirements. De los requirements.txt, yo tengo las versiones:
